  ...
  async getPaymentMethodData(startDate, endDate) {
    const [paymentRows] = await db.execute(`
      SELECT
        ...
        SUM(CASE WHEN status = 'cancelled'       THEN 1    ELSE 0 END)                       AS cancelled,
        SUM(CASE WHEN status != 'cancelled' THEN discount  ELSE 0 END)                       AS total_discounts,
        SUM(CASE WHEN discount > 0 AND status != 'cancelled' THEN 1 ELSE 0 END)              AS discounted_orders
      FROM orders
      WHERE DATE(created_at) BETWEEN ? AND ?
      GROUP BY payment_method
